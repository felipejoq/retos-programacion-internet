...
function fixGiftList(received, expected) {
    const gifCounter = (arr) => arr.reduce((acc, gif) => {
        acc[gif] = (acc[gif] || 0) + 1;
        return acc;
    }, {});

    const expectedGifCounted = gifCounter(expected);
    const receivedGifCounted = gifCounter(received);

    const missing = {}, extra = {}

    for (const gif in expectedGifCounted) {
		const receivedQty = receivedGifCounted[gif] || 0;
		if (receivedQty < expectedGifCounted[gif]) {
			missing[gif] = expectedGifCounted[gif] - receivedQty;
		}
	}

	for (const gif in receivedGifCounted) {
		const expectedQty = expectedGifCounted[gif] || 0;
		if (receivedGifCounted[gif] > expectedQty) {
			extra[gif] = receivedGifCounted[gif] - expectedQty;
		}
	}
